...
public class VoiceCallActivity extends CallActivity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mCardImage=(ImageView)findViewById(R.id.swing_card);
        ...
        info = DbUtil.getInstance().findById(username, FriendInfo.class);

        // 语音电话是否为接收的
        isInComingCall = getIntent().getBooleanExtra("isComingCall", false);

        if (info != null && !info.getNickname().isEmpty()) {
            nickTextView.setText(info.getNickname());
            BitmapUtil.displayImage(mCardImage,info.getHead_link());
        } else {
            nickTextView.setText("匿名");
        }
