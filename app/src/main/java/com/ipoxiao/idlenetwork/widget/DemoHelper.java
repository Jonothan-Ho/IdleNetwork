package com.ipoxiao.idlenetwork.widget;
...
import com.ipoxiao.idlenetwork.framework.db.DemoDBManager;
import com.ipoxiao.idlenetwork.framework.db.InviteMessage;
import com.ipoxiao.idlenetwork.framework.db.InviteMessgeDao;
import com.ipoxiao.idlenetwork.framework.db.UserDao;
...
import com.ipoxiao.idlenetwork.receiver.CallReceiver;
...
public class DemoHelper implements IFriendController {
    @Override
    ...
    public void onComplete(List<FriendInfo> friendInfos) {
        if (friendInfos != null && friendInfos.size() > 0) {
            mFriendInfo = friendInfos.get(0);
            synchronized (DemoHelper.class) {
                if (DbUtil.getInstance().findById(mFriendInfo.getId(), FriendInfo.class) == null) {
                    DbUtil.getInstance().save(mFriendInfo);
                } else {
                    DbUtil.getInstance().update(mFriendInfo);
                }
            }
        }
    }
...
//                }
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotificationInfoProvider() {
            ...
            public String getLatestText(EMMessage message, int fromUsersNum, int messageNum) {
                ...
                message.getType();
                if (user != null) {
                    return user.getNick() + getMsg(message.getType());
                } else {
                    return fromUsersNum + getMsg(message.getType());
                }
//                return null;
            }
...
//                    intent = new Intent(appContext, VoiceCallActivity.class);
                    ...
                    if (chatType == ChatType.Chat) { // 单聊信息
                        intent = new Intent(appContext, PersonChatActivity.class);
                        intent.putExtra("id", message.getFrom());
                        intent.putExtra("chatType", Constant.CHATTYPE_SINGLE);
                    } else { // 群聊信息
                        intent = new Intent(appContext, GroupChatActivity.class);
                        // message.getTo()为群聊id
    ...
    private String getMsg(EMMessage.Type type) {
        String resut = "";
        switch (type) {
            case IMAGE:
                resut = "发来了一张图片!";
...
//            msg.addBody(new TextMessageBody(reason + " " + st3));
//            // 保存邀请消息
...
//
    ...
    private EaseUser getUserInfo(String username) {
        ...
        FriendInfo friendInfo = DbUtil.getInstance().findById(username, FriendInfo.class);
        LogUtil.printf("setUserProfileProvider===>" + friendInfo);
        EaseUser user = null;
        if (friendInfo != null) {
            user = new EaseUser(username);
            User userinfo = PreferencesUtil.getInstance(appContext).get(User.class);
            if (userinfo.getId() == username) {
                user.setAvatar(userinfo.getHead_link());
                user.setNick(userinfo.getNickname());
            } else {
                user.setAvatar(friendInfo.getHead_link());
            ...
            if (mFriendInfo != null) {
                user = new EaseUser(username);
                user.setAvatar(mFriendInfo.getHead_link());
                user.setNick(mFriendInfo.getNickname());
            }
        }
        return user;
    }
                ...
                switch (event.getEvent()) {
                    case EventNewMessage:
                        ...
                        try {
                            ...
                            if (id != null && !id.isEmpty()) {
                                FriendInfo friendInfo = DbUtil.getInstance().findById(id, FriendInfo.class);
                                if (friendInfo == null) {
                                    friendInfo = new FriendInfo();
                                    friendInfo.setId(id);
                                    friendInfo.setHead_link(url);
                                    friendInfo.setNickname(nickname);
                                    DbUtil.getInstance().save(friendInfo);
                                } else {
                                    if (nickname != friendInfo.getNickname() || url != friendInfo.getHead_link()) {
                                        friendInfo.setHead_link(url);
                                        friendInfo.setNickname(nickname);
                                        DbUtil.getInstance().update(friendInfo);
                                    }
                                }
                            }
                            LogUtil.printf("url=========================>" + message.getStringAttribute("url"));
                    ...
                    case EventOfflineMessage:
                        if (!easeUI.hasForegroundActivies()) {
                            ...
                            List<EMMessage> messageList = (List<EMMessage>) event.getData();
                            broadcastManager.sendBroadcast(new Intent(Constant.ACTION_CONTACT_CHANAGED));
        ...
        new Thread() {
            ...
            public void run() {
                ...
                try {
                    ...
                    LogUtil.printf("usernames========================>"+usernames);
